...
This repo is a **web demo**. There is no native Screen Time. During a focus session, tokens and the timer only move while the Catalyst tab is focused. School sites stay conceptually allowed. Tasks are **manual** (title, optional subject and due). Marking a task done is a checklist only — it does not pay tokens. Tokens come from focused study time. School integrations (ManageBac / Google Classroom) are hidden for now; helper APIs remain in the repo unused.
...
5. `/setup` — nemesis apps + required lock hours. No school-provider step. `/integrations` redirects to Focus.
...
9. `/sprite` — My Sprite. Equip UI lists owned looks only. `/appearance` is the Token shop: Sprite shop (cosmetics, sprite colour, sprite gradient, auras / body glow, trails) vs App appearance (focus scenes, rooms). Gold has a metallic shine so it is not lemon. Cosmic is dark + light blue with stars. `/quiz` is a daily check against homework-style captions.
