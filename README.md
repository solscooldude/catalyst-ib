...
Sprite starts as an egg and hatches on the first real focus block or snack, then grows Hatchling → Sparklet → Steady → Bright → Luminary from study time, streak, and care. My Sprite has a quiet care-stage ⓘ and a 10s token-catch minigame. Play-with ⓘ is a centered sheet. Planner lives under Home: to-dos, a week grid, tests and deadlines. First-run intro is six short slides (Skip once, never again). Soft sounds can mute in Account settings. Friends is a top-level tab: minigames / task race, manage add/remove, and a leaderboard.
