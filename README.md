...
Sprite play on Focus and My Sprite: poke a side, boop the face, scrunch the head or belly, double-tap to spin, drag across the belly to tickle, wave the cursor nearby for a mirrored pose, long-press to sleep, feed a snack, catch a mint star. Equip by tapping an owned look on My Sprite — the full catalog stays in Appearance. My Sprite ⓘ lists the short cheat sheet. Focus can sit the sprite beside the timer (study buddy sit). No random high-fives. Heart sunglasses are oversized on the face. Auras are a body-glow colour around the drop (quiz starters plus shop glows) — one at a time.

Sprite starts as an egg and hatches on the first real focus block or snack, then grows Hatchling → Growing → Luminary → Ethereal from study time, streak, and care. Ethereal is the final form after Luminary (care score 64) and brightens the same drop body. The sprite is the original mint drop/droplet blob with accessories on that silhouette. First-run intro is Welcome → problem / tool / focus / sprite / tokens → a 13-question study-vibe quiz (no animal names) that grants a free starter glow (amber, pearl, grove, magenta, hot pink, or mint-teal) and a study-style title, then you name the sprite and finish profile + lock hours (default after school 16:30–19:30) before Home. Retake the quiz from Account settings (title always updates; glow only if you confirm). Reset account lives only at the bottom of Account settings (danger zone + confirm). My Sprite has a quiet care-stage ⓘ and an Avoid falling objects minigame (arrow keys or A/D; survive 12s for a small token bonus). Play-with ⓘ is a centered sheet. Planner lives under Home: to-dos, a week grid, tests and deadlines. Soft sounds can mute in Account settings. Friends is a top-level tab: task races, add/remove with a system-assigned friend code, and a leaderboard ranked by study time this week (Monday–Sunday, local time). Minigames stay on My Sprite.

## Palette

Default chrome is dark (`#0B0B0F` / `#18181B`) with mint `#5EEAD4`. Light mode is pale gray (`#F4F4F5`) with white cards; night-sky stars and pale trails invert so they stay visible. Muted zinc `#A1A1AA`. Aurora is an optional quiet purple room skin, not the default. Headlines use Plus Jakarta Sans; body copy stays Geist. The sprite is the original drop blob in the home room (flux-card walls, teal nest, shelf, lamp, art). Motion (idle bob, poke, breathe, drag) stays CSS. Default name is Sprite. The vibe quiz sets a starter glow; shop auras can change it later.
...
8. The toolbar toggle is On/Off outside lock hours. During lock hours or a live study block it stays forced On. Open YouTube or Instagram while it is On — you should see the Locked page. Docs stays open.
...
12. `/profile` — username, picture, study-style title, diploma subjects, graduating year, and optional college / course. No “why it matters” section.
13. `/account/settings` — Sign-in status, Save to account, Chrome lock extension (same Add extension flow as Setup), school allowlist (defaults + extras), blocked hosts per tier, mute, and retake vibe quiz. `/account/subscription` redirects there. Billing is stubbed. `/locked` is the in-app interstitial the extension can point at.
