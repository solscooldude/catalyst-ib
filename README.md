...
Sprite play on Focus and My Sprite: poke a side, boop the face, scrunch the head or belly, double-tap to spin, drag across the belly to tickle, wave the cursor nearby for a mirrored pose, long-press to sleep, feed a snack, catch a mint star. Equip by tapping an owned look on My Sprite — the full catalog stays in Appearance. My Sprite ℹ lists the short cheat sheet. Focus can sit the sprite beside the timer (study buddy sit). No random high-fives. Heart sunglasses are oversized on the face. Auras include quiz glows (mint-teal, amber, pearl, grove, magenta, hot pink) plus shop colours — one at a time.

Sprite starts as a species egg (Fox flame speckles, Bunny clovers, Deer fawn dots, Cat tabby, Axolotl frills, Dragon scales) and hatches after three snacks — small crack, bigger crack, then Hatchling. It then grows Hatchling → Growing → Luminary → Ethereal from study time, streak, and care. Ethereal is the final form after Luminary (care score 64: official hours + streak + snacks/quiz/catch). The body is a soft SVG animal — Fox, Bunny, Deer, Cat, Axolotl, or Dragon — with chibi proportions, glossy eyes, species-accurate noses, and no heart mark on the cat. Signature Ethereal glows follow the quiz pairing: Fox amber, Bunny pearl pink/lavender, Deer gold + pale green, Cat magenta/violet, Axolotl hot pink, Dragon mint-teal. Extra Magical is a My Sprite / Appearance toggle on Ethereal only (default off). Reset account lives only at the bottom of Account settings (danger zone + confirm). My Sprite has a quiet care-stage ℹ and an Avoid falling objects minigame (arrow keys or A/D; survive 12s for a small token bonus). Play-with ℹ is a centered sheet. Planner lives under Home: to-dos, a week grid, tests and deadlines. First-run intro is Welcome → problem / tool / focus / sprite / tokens → a 7-question study quiz (everyday situations, no animal cues) that assigns a starter animal + glow + study-style title (Night owl sprinter, Social sprinter, Steady marathoner, Deep-dive explorer, Calm planner, Bold challenger). Retake from Account settings; retake always updates the title and asks before changing the animal. Then you name the sprite and finish profile + lock hours (default after school 16:30–19:30) before Home. Soft sounds can mute in Account settings. Friends is a top-level tab: task races, sent/received friend requests (accept / decline / cancel, with a badge for pending received), usernames + profile pictures with the assigned friend ID as secondary text, and a leaderboard ranked by study time this week (Monday–Sunday, local time). Minigames stay on My Sprite.

## Palette

Default chrome is dark (`#0B0B0F` / `#18181B`) with mint `#5EEAD4`. Light mode is pale gray (`#F4F4F5`) with white cards; night-sky stars and pale trails invert so they stay visible. Muted zinc `#A1A1AA`. Aurora is an optional quiet purple room skin, not the default. Headlines use Plus Jakarta Sans; body copy stays Geist. The sprite is a soft SVG animal in a plain My Sprite room — a near-black to slightly lighter vertical wash in the brand palette (shop room colours still tint `--card` / `--background`), a faint floor line, and a soft oval shadow. No shelf, lamp, wall art, or teal nest. Snacks sit in a small tray to the side and only appear when a feed is available. Auras and Ethereal glows are layered radial fades with a wide blur — no hard rings. Species lives on the quiz result (`fox`, `bunny`, `deer`, `cat`, `axolotl`, `dragon`). Motion (idle bob, poke, breathe, drag) stays CSS. Default name is Sprite.

Extra Magical is a switch on My Sprite and Appearance — Ethereal only, off by default. It adds a CSS particle overlay on the SVG glow.
...
7. Click the **Catalyst Lock** toolbar icon for ON / OFF (current version is **0.4.5**), hours, sync, and unlocks. After you replace files, open `chrome://extensions` and press **Reload** on Catalyst Lock
8. The toolbar toggle is free On/Off outside lock hours. During lock hours or a live study block it stays forced On even if it was stored Off. Open Instagram (or YouTube if it is your nemesis) while it is On — you should see the Locked page. Docs stays open. The popup shows **Update available** if Setup has a newer zip.
...
14. `/friends` — top-level tab. Task races (same manual task), friend requests (Received with Accept/Decline, Sent with Cancel), friends list + send a request by assigned code (username + picture everywhere; the code is small secondary text), and a leaderboard ranked by study time this week (Monday–Sunday, your local time). Avoid falling objects lives on My Sprite, not here.
