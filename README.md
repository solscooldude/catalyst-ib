...
This repo is a **web demo**. There is no native Screen Time. During a focus session, tokens and the timer only move while the Catalyst tab is focused. School sites stay conceptually allowed. ManageBac has no public student API — paste a list, ICS, or load sample tasks. Google Classroom can sync live if Sols adds a personal Google Cloud OAuth client.
...
5. `/setup` — nemesis apps + mock ManageBac + required lock hours. `/integrations` connects Google Classroom (OAuth when `GOOGLE_CLIENT_ID` / `GOOGLE_CLIENT_SECRET` are set, otherwise sample tasks) and ManageBac (paste/ICS/sample — no scrape). Tasks feed Focus, quiz captions, and the completion bonus.
...
14. `/friends` — top-level tab. Choose your own unique code (4–12 letters/numbers/hyphens). Minigames (task race + Avoid falling objects — minigame), manage friends (add/remove), and a leaderboard ranked by study minutes, then tasks completed.
