...
10. `/appearance` — closet collections (rooms parked). `/sprite` is My Sprite: visual previews, pet/feed/poke/high-five/drag/sleep. Daily login streak pays 1 token; 7 days unlocks Seven-day flare. `/quiz` is a 3-question subject check (tokens for correct answers, once a day). Focus is a large spark on a quiet stage (pet, wander, morph, loop, rare +1 token).
