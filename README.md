...
Sprite starts as an egg and hatches on the first real focus block or snack, then grows Hatchling → Growing → Luminary → Ethereal from study time, streak, and care. Ethereal is the final form after Luminary (care score 64: official hours + streak + snacks/quiz/catch). The body is a sitting SVG chibi with natural species colours — no image plate. Signature Ethereal glows are SVG/CSS (never a shared mint wash): Fox amber/sunset, Bunny pearl-pink/lavender, Deer gold + pale green, Cat magenta/violet, Axolotl hot pink, Dragon mint-teal (mint reserved for Dragon + brand UI). Extra Magical is a My Sprite / Appearance toggle on Ethereal only (default off) — a CSS particle overlay on the SVG glow, not fox-only eye spots. Reset account lives only at the bottom of Account settings (danger zone + confirm). My Sprite has a quiet care-stage ⓘ and an Avoid falling objects minigame (arrow keys or A/D; survive 12s for a small token bonus). Play-with ⓘ is a centered sheet. Planner lives under Home: to-dos, a week grid, tests and deadlines. First-run intro is Welcome → problem / tool / focus / sprite / tokens → a 13-question pet quiz that assigns Fox, Bunny, Deer, Cat, Axolotl, or Dragon (options never show animal names; ties ask which feels more you, or the Axolotl/Dragon drain prompt), then you name the sprite and finish profile + lock hours (default after school 16:30–19:30) before Home. Soft sounds can mute in Account settings. Friends is a top-level tab: task races, sent/received friend requests (accept / decline / cancel, with a badge for pending received), usernames + profile pictures with the assigned friend ID as secondary text, and a leaderboard ranked by study time this week (Monday–Sunday, local time). Minigames stay on My Sprite.

## Palette

Default chrome is dark (`#0B0B0F` / `#18181B`) with mint `#5EEAD4`. Light mode is pale gray (`#F4F4F5`) with white cards; night-sky stars and pale trails invert so they stay visible. Muted zinc `#A1A1AA`. Aurora is an optional quiet purple room skin, not the default. Headlines use Plus Jakarta Sans; body copy stays Geist. The sprite is a sitting SVG chibi (huge head, tiny body) in the home room — transparent, no grey plate. Quiz and Profile species (`fox`, `bunny`, `deer`, `cat`, `axolotl`, `dragon`) pick the matching SVG palette. Motion (idle bob, poke, breathe, drag) stays CSS. Default name is Sprite.
...
7. Click the **Catalyst Lock** toolbar icon for ON / OFF (current version is **0.4.0**), hours, sync, and unlocks. After you replace files, open `chrome://extensions` and press **Reload** on Catalyst Lock
