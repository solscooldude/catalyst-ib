...
The extension lives in `extension/`. It does not need a backend. Keep the Catalyst tab open so it can write lock hours, unlocks, and the allowlist into `chrome.storage`.

1. Open Chrome → `chrome://extensions`
2. Turn on **Developer mode**
3. Click **Load unpacked**
4. Select the `extension/` folder in this repo
5. Sign in on [https://catalyst-ib.vercel.app](https://catalyst-ib.vercel.app) (or local `http://127.0.0.1:43127`) so policy syncs
6. During lock hours, open YouTube or Instagram — you should see the Locked page. Docs stays open.
...
13. `/account/settings` — school allowlist (defaults + extras), blocked hosts per tier, account basics, mute. `/account/subscription` redirects there. Billing is stubbed; this demo is free. `/locked` is the in-app interstitial the extension can point at.
