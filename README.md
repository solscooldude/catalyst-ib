...
5. `/focus` — choose a ManageBac task (dark text on light cards; selected task gets a mint border, check, and a “Selected task” chip) or start a study block. Demo speed is on by default: **30 seconds = 1 token** (real pace is 5 minutes).
6. `/lock` — simulated lock screen. Emergency stays; social apps are grey.
7. `/session` — timer counts up (0:00 → 24:18). Pause or End whenever you want. Official ManageBac tasks still need the done tick. Study blocks do not auto-end.
...
13. Study blocks (from Focus or Stats) lock the phone, then count up until you End. Optional soft goal only. Time tokens only — no +5. Retroactive “log past study” is gone. After End, Unlocks shows a recap (minutes, tokens, Spark celebrate). Home shows today’s quiet hours from Setup. Focus stage (night sky / sea / math) is remembered on this device. Shop and My Sprite let you try closet pieces on Spark before buying.
