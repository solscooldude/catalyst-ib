...
function PetQuizPage() {
  ...
  const [applyGlow, setApplyGlow] = useState(!retake);
  ...
  function finishQuestion(optionIndex: number, index: number) {
    ...
    if (outcome.kind === "winner") {
      const style = outcome.styles[0] ?? "night-owl";
  ...
  function continueSetup() {
    ...
    completePetQuiz(step.style, { applyGlow: retake ? applyGlow : true });
    if (!retake) {
...
function RevealStep({
  ...
  showName: boolean;
  applyGlow: boolean;
  onApplyGlow: (value: boolean) => void;
  confirmGlow: boolean;
  onContinue: () => void;
  ...
  return (
    <>
      ...
      <h1 className="mt-4 text-3xl font-semibold tracking-tight text-zinc-50 sm:text-4xl">
        Your sprite glows {card.glowName}
      </h1>
      <p className="mt-3 text-lg font-medium text-[#5EEAD4]">{card.title}</p>
      <p className="mt-2 text-sm leading-relaxed text-zinc-400">{card.line}</p>
      <div className="mt-6 flex justify-center">
        <Spark mood="done" size={168} evolve={false} aura={card.aura} />
      </div>
      {confirmGlow ? (
        <label className="mt-6 flex items-start gap-3 text-sm text-zinc-300">
          <input
            type="checkbox"
            checked={applyGlow}
            onChange={(event) => onApplyGlow(event.target.checked)}
            className="mt-1 size-4 accent-[#5EEAD4]"
          />
          <span>
            Also switch my glow to {card.glowName}. Leave unchecked to keep the
            glow you already equipped.
          </span>
        </label>
      ) : null}
      {showName ? (
