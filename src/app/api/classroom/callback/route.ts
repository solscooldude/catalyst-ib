...
export async function GET(request: NextRequest) {
  ...
  if (!classroomCredentials().ready) {
    return Response.redirect(`${origin}/focus?classroom=needs-credentials`);
  }
  const error = request.nextUrl.searchParams.get("error");
  const code = request.nextUrl.searchParams.get("code");
  if (error || !code) {
    return Response.redirect(`${origin}/focus?classroom=denied`);
  }
  try {
    ...
    return Response.redirect(`${origin}/focus?classroom=ok`);
  } catch {
    return Response.redirect(`${origin}/focus?classroom=error`);
  }
}
